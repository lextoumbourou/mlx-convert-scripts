"""Convert MobileNetV3 PyTorch weights to MLX format.

This script downloads PyTorch MobileNetV3 weights (or uses existing ones),
loads them into an MLX MobileNetV3 model, then saves the MLX model's weights
as safetensors.
"""
import argparse
import sys
...
from mlxim.model.mobilenet.mobilenetv3 import mobilenet_v3_large, mobilenet_v3_small
...
def load_pytorch_weights(model, weights_path: Union[str, Path]) -> None:
    ...
    # Helper function to load Squeeze-Excitation layer
    def load_se_layer(mlx_se_layer, pt_prefix):
        """Load weights for a Squeeze-Excitation layer."""
        # FC1 (avgpool.0)
        if f"{pt_prefix}.fc1.weight" in pt_weights:
            mlx_se_layer.fc1.weight = transpose_conv(pt_weights[f"{pt_prefix}.fc1.weight"])
            if f"{pt_prefix}.fc1.bias" in pt_weights:
                mlx_se_layer.fc1.bias = mx.array(pt_weights[f"{pt_prefix}.fc1.bias"])

        # FC2 (avgpool.2)
        if f"{pt_prefix}.fc2.weight" in pt_weights:
            mlx_se_layer.fc2.weight = transpose_conv(pt_weights[f"{pt_prefix}.fc2.weight"])
            if f"{pt_prefix}.fc2.bias" in pt_weights:
                mlx_se_layer.fc2.bias = mx.array(pt_weights[f"{pt_prefix}.fc2.bias"])
    ...
    # Load inverted residual blocks (features.1 through features.N-1)
    # where N-1 is the last conv before classifier
    num_blocks = len(features_layers) - 2  # Exclude stem and last conv

    for block_idx in range(1, num_blocks + 1):
        pt_block_prefix = f"features.{block_idx}.block"
        mlx_block = features_layers[block_idx]

        # Each InvertedResidual block has a block Sequential with variable layers
        ...
        if first_conv_key in pt_weights:
            ...
            # Depthwise conv: (out, 1, k, k) - in_channels/groups = 1
            # Expansion conv: (out, in, 1, 1) - where in > 1
            has_expansion = first_conv_shape[1] > 1 and first_conv_shape[2] == 1
        else:
            has_expansion = False

        mlx_block_layers = mlx_block.block.layers
        pt_layer_idx = 0
        mlx_layer_idx = 0

        if has_expansion:
            # Structure: expand (Conv+BN+Act), depthwise (Conv+BN+Act), [SE], project (Conv+BN)
            # Expansion layer (pointwise)
            load_conv_norm_block(mlx_block_layers[mlx_layer_idx], f"{pt_block_prefix}.{pt_layer_idx}")
            pt_layer_idx += 1
            mlx_layer_idx += 1
        ...
        # Check for Squeeze-Excitation layer
        se_key = f"{pt_block_prefix}.{pt_layer_idx}.fc1.weight"
        if se_key in pt_weights:
            # Load SE layer
            load_se_layer(mlx_block_layers[mlx_layer_idx], f"{pt_block_prefix}.{pt_layer_idx}")
            pt_layer_idx += 1
            mlx_layer_idx += 1

        # Project layer (pointwise-linear) - Conv+BN (no activation)
        load_conv_norm_block(mlx_block_layers[mlx_layer_idx], f"{pt_block_prefix}.{pt_layer_idx}")

    # Load final conv layer (last layer in features)
    last_conv_idx = len(features_layers) - 1
    load_conv_norm_block(features_layers[last_conv_idx], f"features.{last_conv_idx}")

    # Load classifier (4 layers: Linear, Hardswish, Dropout, Linear)
    # classifier.0 - Linear
    if "classifier.0.weight" in pt_weights:
        model.classifier.layers[0].weight = mx.array(pt_weights["classifier.0.weight"])
        model.classifier.layers[0].bias = mx.array(pt_weights["classifier.0.bias"])

    # classifier.3 - Linear (skip 1=Hardswish, 2=Dropout)
    if "classifier.3.weight" in pt_weights:
        model.classifier.layers[3].weight = mx.array(pt_weights["classifier.3.weight"])
        model.classifier.layers[3].bias = mx.array(pt_weights["classifier.3.bias"])
...
def generate_readme(model_name: str, output_dir: Path, img_size: int) -> None:
    ...
    """
    model_desc = "MobileNetV3-Large" if "large" in model_name else "MobileNetV3-Small"
    readme_content = f"""---
license: apache-2.0
...
A {model_desc} model architecture, pretrained on ImageNet-1K.
...
def convert_pytorch_to_mlx(pytorch_weights_path: Path, output_path: Path, model_name: str = "mobilenet_v3_large"):
    """Convert PyTorch MobileNetV3 weights to MLX format.
    ...
    model_configs = {
        "mobilenet_v3_large": {"img_size": 224},
        "mobilenet_v3_small": {"img_size": 224},
    }

    # Create MLX model
    if model_name == "mobilenet_v3_large":
        model = mobilenet_v3_large(num_classes=1000, dropout=0.2)
    elif model_name == "mobilenet_v3_small":
        model = mobilenet_v3_small(num_classes=1000, dropout=0.2)
    else:
...
def download_pytorch_weights(model_name: str, weights_dir: Path) -> Path:
    ...
    Args:
        model_name: Name of the model (e.g., 'mobilenet_v3_large')
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert MobileNetV3 PyTorch weights to MLX format"
    )
    parser.add_argument(
        "--model",
        type=str,
